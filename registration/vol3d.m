...
%my stuff
MY_DIR = '/usr/local/hdd/mouse_pictures/sequences/4';
files = dir(fullfile(MY_DIR, '*.tif'));
input = int16.empty;

for k = 1:(length(files)-1)
    name1 = fullfile(MY_DIR, files(1).name);
    name2 = fullfile(MY_DIR, files(k+1).name);
    file1 = Tiff(name1);
    file2 = Tiff(name2);
    fixed = read(file1);
    moving = read(file2);
    ...
    imwrite(movingRegistered, [name2, '.reg.tiff'])
    if k==1
        input(:,:,k,:) = fixed;
        
    end
    input(:,:,k+1,:) = movingRegistered;
end

%load mri.mat
vol3d('cdata', squeeze(input));
colormap(bone(256));
