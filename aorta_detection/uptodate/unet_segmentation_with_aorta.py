...
model.compile(optimizer=Adam(lr=1e-5), loss=per_pixel_softmax_loss)
print(model.summary(line_length=135))


history = model.fit(train_image, train_label, batch_size=1, epochs=250, verbose=1, validation_split=0.15)

# serialize model to JSON
model_json = model.to_json()
with open("model_softmax_hist_640_ZT113_250.json", "w") as json_file:
    json_file.write(model_json)
# serialize weights to HDF5
model.save_weights("model_softmax_640_ZT113_250.h5")
